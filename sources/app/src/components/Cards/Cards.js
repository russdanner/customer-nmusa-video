...
class Cards extends Component {
  ...
  searchCards(props) {
    ...
    if (category.hits) {
      return this.setState({ hits: props.category.hits });
    }
  ...
  searchWithSize(category, from = 0, size) {
    const queryBuilder = SearchService.createQuery('elasticsearch');
    ...
    SearchService
      ...
      .subscribe(response => {
        this.setState({ hits: response.hits, total: response.total });
      });
  }
