...
class VideoJSPlayer extends Component {
  ...
  setPlayerSrc(player, video) {
    ...
    if (contentType === '/component/youtube-video') {   // YOUTUBE
      ...
      // add caption
      const track = video.video_o.item.length > 0 ? video.video_o.item[0].url.replace(/\.[^.]*$/,'.vtt') : '';
      if (track) {
        player.addRemoteTextTrack({ src: track, type: 'captions', srclang: 'en', label: 'English', default: true });
      }
